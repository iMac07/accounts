    ...
    public void test01NewTransaction(){
        ...
        try {
            if (_trans.NewTransaction()){
                ...
                _trans.setMaster("sClientID", "000122000006");
                _trans.setMaster("sRemarksx", "This is a test payment.");
                ...
                if (_trans.LoadTransaction()){
                    ...
                    for (int lnCtr = 0; lnCtr <= _trans.getItemCount()-1; lnCtr++){
                        ...
                        System.out.println(StringUtil.NumberFormat((double) _trans.getDetail(lnCtr, "nDebitAmt"), "#,##0.00"));
                        System.out.println(StringUtil.NumberFormat((double) _trans.getDetail(lnCtr, "nCredtAmt"), "#,##0.00"));
                        System.out.println(StringUtil.NumberFormat((double) _trans.getDetail(lnCtr, "nAppliedx"), "#,##0.00"));
                        System.out.println((String) _trans.getDetail(lnCtr, "xDescript"));
                        System.out.println((String) _trans.getDetail(lnCtr, "xReferNox"));
                        System.out.println(SQLUtil.dateFormat((Date) _trans.getDetail(lnCtr, "dTransact"), SQLUtil.FORMAT_MEDIUM_DATE));
                        System.out.println(SQLUtil.dateFormat((Date) _trans.getDetail(lnCtr, "dDueDatex"), SQLUtil.FORMAT_MEDIUM_DATE));
                        
                        if ("PR".equals((String) _trans.getDetail(lnCtr, "sSourceCd")))
                            _trans.setDetail(lnCtr, "nAppliedx", (double) _trans.getDetail(lnCtr, "nCredtAmt"));
                        else
                            _trans.setDetail(lnCtr, "nAppliedx", (double) _trans.getDetail(lnCtr, "nDebitAmt"));
                    }
                    
                    if (!_trans.SaveTransaction()) fail(_trans.getMessage());
                } else {
